...
from utils.general_methods import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    fitness, strip_optimizer, get_latest_run, check_dataset, check_file, check_git_status, check_img_size, \
    print_mutation, set_logging
from utils.google_utils import attempt_download
from utils.loss_calc import compute_loss
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if rank in [-1, 0]:
        for f in [last, best]:
            if f.exists():
                strip_optimizer(f)
                os.system('gsutil cp %s gs://%s/weights' % (f, opt.bucket)) if opt.bucket else None
